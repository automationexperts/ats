...
void setup()
{
  ...
  pinMode(Pin,INPUT_PULLUP);   // pin is at +5V normally unless pulled low by another device
}

void loop()
{
  state = digitalRead(Pin);
  if (state != LastState)  //only react when instruction changes, this prevents lots of codes from being sent to the machine constantly
  {
	  if (state == 1)  //turn off motor
	  {
		  Turn_const_speed(driveID, 0); delay(1);
		  Turn_const_speed(driveID, 0); delay(1);
		  Turn_const_speed(driveID, 0); delay(1);
		  Turn_const_speed(driveID, 0); delay(1);
		  Turn_const_speed(driveID, 0); delay(1);
	  } else {   //turn on motor
		  Turn_const_speed(driveID, speed); delay(1);	
		  Turn_const_speed(driveID, speed); delay(1);
		  Turn_const_speed(driveID, speed); delay(1);
		  Turn_const_speed(driveID, speed); delay(1);
		  Turn_const_speed(driveID, speed); delay(1);
